...
//宏定义
#define BLOCK_SIZE	512						//block块大小为512B
#define INODE_SIZE	128						//inode节点大小为128B。注：sizeof(Inode)不能超过该值
#define MAX_NAME_SIZE 28					//最大名字长度，长度要小于这个大小

#define INODE_NUM	640						//inode节点数,最多640个文件
#define BLOCK_NUM	10240					//block块数，10240 * 512B = 5120KB，外存总大小
#define BLOCKS_PER_GROUP	64				//空闲块堆栈大小，一个空闲堆栈最多能存多少个单独磁盘块地址，成组链接法
...
#define FILESYSNAME	"FileSystem.sys"			//虚拟磁盘文件名


//结构体声明
//超级块
struct SuperBlock{
    unsigned short s_INODE_NUM;				//inode节点数，最多 65535，实际上有640个inode
    unsigned int s_BLOCK_NUM;				//磁盘块块数，最多 4294967294，实际上有10240个block

    unsigned short s_free_INODE_NUM;		//空闲inode节点数
    unsigned int s_free_BLOCK_NUM;			//空闲磁盘块数
    int s_free_addr;						//空闲块堆栈指针，作为索引地址
    int s_free[BLOCKS_PER_GROUP];			//空闲块堆栈，用来做成组链接法管理空闲block块

    unsigned short s_BLOCK_SIZE;			//磁盘块大小 512B
    unsigned short s_INODE_SIZE;			//inode大小 128B
    unsigned short s_SUPERBLOCK_SIZE;		//超级块大小
    unsigned short s_blocks_per_group;		//每个 blockgroup 的block数量 todo 160

    //磁盘分布
    int s_Superblock_StartAddr;             //超级块起始地址
    int s_InodeBitmap_StartAddr;            //inode位图起始地址
    int s_BlockBitmap_StartAddr;            //磁盘block块起始地址
    int s_Inode_StartAddr;                  //inode区起始地址
    int s_Block_StartAddr;                  //磁盘block区起始地址
};
...
struct Inode{
    ...
    unsigned short i_cnt;					//链接数。有多少文件名指向这个inode todo 树形结构tree
...
extern const int Inode_StartAddr;			//inode节点区 偏移地址，占 INODE_NUM/(BLOCK_SIZE/INODE_SIZE) 个磁盘块，640/(512/128)=160
extern const int Block_StartAddr;			//block数据区 偏移地址 ，占 BLOCK_NUM 个磁盘块,10240*512B=5120KB=5MB
extern const int File_Max_Size;				//单个文件最大大小
extern const int Sum_Size;					//虚拟磁盘文件总大小


//全局变量声明
extern int Root_Dir_Addr;					//根目录inode地址
extern int Cur_Dir_Addr;					//当前目录
extern int Tree_Dir_Addr;                   //tree专用
extern int Cur_Dir_Addr1;                   //findfile专用
extern char Cur_Dir_Name[310];				//当前目录名
extern char Cur_Host_Name[110];				//当前主机名，服务器名
...
extern bool block_bitmap[BLOCK_NUM];		//磁盘block块位图
